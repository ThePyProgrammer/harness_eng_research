...
1. **RESOLVED: Should the explicit inventory be TypeScript or JSON loaded through Astro content collections?**
   - Decision: Inventory is TypeScript-first under `site/src/data/corpus.ts`, validated with Zod at build/validation time through `site/src/data/corpus.schema.ts`.
   - Rationale: Context requires explicit typed data under `site/`; direct TypeScript literal data gives richer compile-time contracts for Phase 1 validation while generated JSON indexes can still be emitted for static consumers. [VERIFIED: /home/prannayag/harness_eng/.planning/phases/01-site-foundation-and-provenance-contract/01-CONTEXT.md]

2. **RESOLVED: Should Phase 1 include Pagefind generation if search UX is Phase 4?**
   - Decision: Phase 1 includes Pagefind/static local index wiring only as static artifacts generated by the documented command, plus `dist/corpus-index.json` from the validated inventory. Full search UX, tuning, and rich indexing remain Phase 4.
   - Rationale: FOUND-07 requires reproducible static HTML assets and local indexes, while search interaction design is explicitly later scope. [VERIFIED: /home/prannayag/harness_eng/.planning/REQUIREMENTS.md] [VERIFIED: /home/prannayag/harness_eng/.planning/ROADMAP.md]

3. **RESOLVED: Should link validation be included now or deferred to Phase 5?**
   - Decision: Full external link validation is deferred to Phase 5. Phase 1 validates only canonical corpus paths and archive/provenance rules.
   - Rationale: Phase 1 is the provenance contract; adding broader link validation now would distract from FOUND-05/FOUND-06 and duplicates Phase 5 scope. [VERIFIED: /home/prannayag/harness_eng/.planning/ROADMAP.md]
