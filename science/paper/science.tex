...
We present a unified formal framework presenting eleven architectural dimensions: seven core pillars (Abstraction, Information, Reliability, Coordination, Temporal, Quality, Governance) addressing the semantic, execution, and assurance axes; three operational pillars (Economics, Human Interaction, Model Routing) addressing the practical constraints that bind deployment; and a Security pillar addressing the containment problem, since agents operate with developer credentials and the structural enforcement principle is fundamentally a security principle.
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item \textbf{Security}: capability-based access control for agents, sandbox containment via the HRU model, information flow control adapted from Bell-LaPadula, credential isolation, adversarial robustness against reward hacking and prompt injection, and the structural enforcement boundary as a security principle.
\end{enumerate}

\paragraph{Why eleven pillars, and how they decompose.} The pillars decompose the harness design space along five axes. The \emph{semantic axis} (Abstraction, Information) addresses what the agent needs to know. The \emph{execution axis} (Reliability, Coordination, Temporal) addresses how the agent acts. The \emph{assurance axis} (Quality, Governance) addresses how outcomes are verified and architectural coherence is maintained. The \emph{operational axis} (Economics, Human Interaction, Model Routing) addresses the practical constraints that bind deployment: finite budgets, finite human attention, and heterogeneous model capabilities. The \emph{security axis} (Security) addresses the containment problem: agents operate with developer credentials, execute arbitrary code, and interact with external services, making security not an afterthought but a foundational constraint. The structural enforcement principle, the paper's strongest cross-pillar result, is fundamentally a security principle: invariants that must hold regardless of agent behavior can only be guaranteed through structural mechanisms, not instructions. The first seven pillars were developed from formal first principles; the operational three were identified through peer review; the security pillar was added after recognition that the structural enforcement boundary, credential isolation, and adversarial robustness concerns demanded a dedicated formal treatment grounded in access control theory, information flow control, and capability-based security. We do not claim that this decomposition is the only productive one, nor that it is exhaustive: user interface design, for example, is not addressed. We claim that this decomposition is \emph{productive}, not that it is complete.
...
\begin{tikzpicture}[
    % Node styles
    pillar/.style={
        rectangle, rounded corners=4pt, draw=#1!70, fill=#1!8,
        minimum width=3.0cm, minimum height=1.05cm,
        font=\small\sffamily, align=center, text=#1!85!black,
        line width=0.8pt
    },
    axislabel/.style={
        font=\footnotesize\sffamily\bfseries, text=#1!70!black
    },
    axisbox/.style={
        rounded corners=8pt, draw=#1!25, fill=#1!3,
        line width=0.5pt, inner sep=9pt
    },
    % Arrow styles
    flow/.style={
        -{Stealth[length=3pt,width=2.5pt]}, line width=0.7pt,
        draw=gray!55, shorten >=2pt, shorten <=2pt
    },
    bridge/.style={
        -{Stealth[length=3pt,width=2.5pt]}, line width=0.7pt,
        draw=orange!65!black, densely dashed, shorten >=2pt, shorten <=2pt
    },
    secline/.style={
        -{Stealth[length=3.5pt,width=2.5pt]}, line width=1.1pt,
        draw=red!55!black, shorten >=2pt, shorten <=2pt
    },
    elabel/.style={
        font=\tiny\sffamily\itshape, text=gray!65, fill=white,
        inner sep=1pt, outer sep=0pt
    },
]

% ===== NODE PLACEMENT =====
% Layout: core pillars on left (3 rows), operational on right, security spanning bottom

% Row 1: Semantic
\node[pillar=blue!80!black] (abs) at (0, 0) {Abstraction\\[-2pt]{\scriptsize$\mathcal{G}(S,P) = K(P \mid S)$}};
\node[pillar=blue!80!black] (inf) at (3.8, 0) {Information\\[-2pt]{\scriptsize$H(P \mid S, C, \theta)$}};

% Row 2: Execution (more vertical space from Semantic)
\node[pillar=teal!90!black] (rel) at (-0.6, -2.8) {Reliability\\[-2pt]{\scriptsize$R = p^n$, elasticity $n$}};
\node[pillar=teal!90!black] (crd) at (3.2, -2.8) {Coordination\\[-2pt]{\scriptsize$n^* \approx 1/\sqrt{\delta_a}$}};
\node[pillar=teal!90!black] (tmp) at (7.0, -2.8) {Temporal\\[-2pt]{\scriptsize VIH $= \lambda \cdot p_v$}};

% Row 3: Assurance (generous space from Execution)
\node[pillar=violet] (qlt) at (1.0, -5.6) {Quality\\[-2pt]{\scriptsize 18 types, 3 classes}};
\node[pillar=violet] (gov) at (4.8, -5.6) {Governance\\[-2pt]{\scriptsize$R_{\text{drift}} < C_{\text{gov}}$}};

% Right column: Operational (aligned with core rows)
\node[pillar=orange!90!black] (eco) at (11.2, 0) {Economics\\[-2pt]{\scriptsize CVIH, budget alloc.}};
\node[pillar=orange!90!black] (hum) at (11.2, -2.8) {Human Interaction\\[-2pt]{\scriptsize Triage, trust, $\tau^*$}};
\node[pillar=orange!90!black] (rte) at (11.2, -5.6) {Model Routing\\[-2pt]{\scriptsize$\Delta_{\text{div}}$, cascade}};

% Bottom: Security (full span, more space above)
\node[pillar=red!80!black, minimum width=13.2cm, minimum height=1.05cm] (sec) at (5.6, -8.2)
    {Security\\[-2pt]{\scriptsize Capability lattice \,$\cdot$\, $(1{-}\epsilon)^T$ containment \,$\cdot$\, Bell-LaPadula flow control}};

% ===== AXIS BOXES (background) =====
\begin{scope}[on background layer]
    \node[axisbox=blue!80!black, fit=(abs)(inf), inner sep=10pt] (sembox) {};
    \node[axisbox=teal!90!black, fit=(rel)(crd)(tmp), inner sep=10pt] (exebox) {};
    \node[axisbox=violet, fit=(qlt)(gov), inner sep=12pt] (assbox) {};
    \node[axisbox=orange!90!black, fit=(eco)(hum)(rte), inner sep=10pt] (opsbox) {};
\end{scope}

% ===== AXIS LABELS (placed above boxes, not overlapping content) =====
\node[axislabel=blue!80!black, anchor=south west] at ([yshift=2pt]sembox.north west) {SEMANTIC};
\node[axislabel=teal!90!black, anchor=south west] at ([yshift=2pt]exebox.north west) {EXECUTION};
\node[axislabel=violet, anchor=south west] at ([yshift=2pt]assbox.north west) {ASSURANCE};
\node[axislabel=orange!90!black, anchor=south west] at ([yshift=2pt]opsbox.north west) {OPERATIONAL};
\node[axislabel=red!80!black, anchor=south west] at ([xshift=4pt, yshift=2pt]sec.north west) {SECURITY};

% ===================================================================
% CONNECTIONS: all use out/in angles or bend to avoid crossing nodes
% ===================================================================

% --- INTRA-AXIS (solid gray, short hops within a row, no labels to reduce clutter) ---
\draw[flow] (abs) -- (inf);
\draw[flow] (rel) -- (crd);
\draw[flow] (crd) -- (tmp);
\draw[flow] (qlt) -- (gov);

% --- INTER-AXIS VERTICAL (solid gray, curved, minimal labels) ---
% Semantic -> Execution
\draw[flow] (abs.south) to[out=-80, in=110] (rel.north);
\draw[flow] (inf.south) to[out=-90, in=90] (crd.north);

% Execution -> Assurance
\draw[flow] (rel.south) to[out=-80, in=100] (qlt.north);
\draw[flow] (crd.south) to[out=-80, in=100] (gov.north);

% --- OPERATIONAL BRIDGES (dashed orange, curved to route around) ---
% Economics -> Information (budget caps context)
\draw[bridge] (eco.west) to[out=180, in=0]
    node[elabel, above, pos=0.5] {budget caps context} (inf.east);

% Economics -> Reliability (cost of verification) -- curve above execution row
\draw[bridge] (eco.south west) to[out=210, in=30]
    node[elabel, above, pos=0.45] {cost of verification} (tmp.north east);

% Human -> Temporal (human in cycle)
\draw[bridge] (hum.west) to[out=180, in=0]
    node[elabel, above, pos=0.5] {human $\in$ cycle} (tmp.east);

% Human -> Quality (Layer 4 review) -- curve below to avoid Governance
\draw[bridge] (hum.south west) to[out=210, in=10]
    node[elabel, below, pos=0.55] {Layer 4 review} (qlt.east);

% Routing -> Governance (diversity gain)
\draw[bridge] (rte.west) to[out=180, in=0]
    node[elabel, above, pos=0.5] {diversity gain} (gov.east);

% Routing -> Coordination (cross-model rho) -- curve up and left
\draw[bridge] (rte.north west) to[out=150, in=-15]
    node[elabel, above right, pos=0.35] {cross-model $\rho$} (crd.east);

% --- SECURITY CONTAINMENT (thick red, curved up from bottom) ---
% Security -> Reliability (far left, wide arc to avoid Quality)
\draw[secline] ([xshift=-5.0cm]sec.north) to[out=110, in=-130]
    node[elabel, left, pos=0.35] {structural} (rel.south west);

% Security -> Quality
\draw[secline] ([xshift=-1.8cm]sec.north) to[out=90, in=-90]
    node[elabel, right, pos=0.45] {enforces} (qlt.south);

% Security -> Governance
\draw[secline] ([xshift=0.5cm]sec.north) to[out=90, in=-90]
    node[elabel, right, pos=0.45] {contains} (gov.south);

% Security -> Model Routing (far right)
\draw[secline] ([xshift=4.5cm]sec.north) to[out=80, in=-80]
    node[elabel, right, pos=0.4] {isolates} (rte.south);

% ===== LEGEND =====
\node[anchor=north west, font=\scriptsize\sffamily, text=gray!65] at (-1.6, -9.5) {%
    \begin{tabular}{@{}l@{\hskip 10pt}l@{\hskip 10pt}l@{}}
    \tikz[baseline=-0.5ex]{\draw[-{Stealth[length=3pt,width=2pt]}, line width=0.7pt, gray!55] (0,0) -- (0.8,0);} Intra/inter-axis &
    \tikz[baseline=-0.5ex]{\draw[-{Stealth[length=3pt,width=2pt]}, line width=0.7pt, densely dashed, orange!65!black] (0,0) -- (0.8,0);} Operational bridge &
    \tikz[baseline=-0.5ex]{\draw[-{Stealth[length=3.5pt,width=2.5pt]}, line width=1.1pt, red!55!black] (0,0) -- (0.8,0);} Security containment
    \end{tabular}
};

\end{tikzpicture}
\caption{The eleven pillars organized by five axes. The core pillars (left) flow top-to-bottom: specification (semantic), through execution, to assurance. The operational pillars (right) impose practical constraints. The security pillar (bottom) provides the structural containment boundary. All cross-axis connections use curved paths; edge labels indicate the primary mechanism at each bridge.}
\label{fig:pillar-architecture}
\end{figure*}
...
\begin{enumerate}
    ...
    \item \textbf{Design principles} grounded in specific formal results across eleven dimensions, including principles for the operational pillars (equal marginal rate, risk-ranked triage, cross-model diversity) and the security pillar (principle of least authority, credential isolation, structural containment).
\end{enumerate}
