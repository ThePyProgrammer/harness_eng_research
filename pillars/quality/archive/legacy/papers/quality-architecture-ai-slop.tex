...
\title{Quality Architecture for AI-Generated Code:\\
A Formal Framework for Layered Defense Against\\
Superficially Competent Defects}

\author{HTX Harness Engineering}
\date{April 2026}
...
\item A \textbf{formal slop taxonomy} (Section~\ref{sec:taxonomy}) classifying 18 defect types into three decidability classes (Decidable, Semi-decidable, Undecidable), with three approximately orthogonal generative factors (Context Blindness, Completion Bias, Training Distribution Leakage).

\item The \textbf{Layered Defense Optimization Problem} (Section~\ref{sec:optimization}), proved NP-hard, with a greedy $(1-1/e)$-approximation via submodularity.

\item The \textbf{Detection Ceiling Theorem} (Section~\ref{sec:ceilings}), proving information-theoretic upper bounds on per-layer detection rates using the Data Processing Inequality.

\item The \textbf{Correlated Judge-Producer Error Model} (Section~\ref{sec:correlation}), showing that shared training data creates positively correlated failures between producer and judge LLMs.

\item The \textbf{Compound Degradation Theorem} (Section~\ref{sec:degradation}), proving superlinear entropy growth from individually CI-passing changes, with the \emph{Accretion Category} as a novel defect class.

\item \textbf{Empirical calibration} (Section~\ref{sec:calibration}) of all formal model parameters against eight large-scale studies, yielding a cost-optimal four-layer quality stack with per-type layer assignments.
\end{enumerate}
...
The 18 slop types partition as shown in Table~\ref{tab:partition}.
\end{proposition}
...
Table~\ref{tab:detection} presents estimated detection probabilities $d_{\ell,j}$ using ranges (not point values) with confidence levels. Confidence: \textbf{H} = strong empirical backing; \textbf{M} = single source or derived; \textbf{L} = extrapolated; \textbf{E} = estimated from first principles.
...
\item \textbf{AgentSpec:} 87\% code-based vs.\ 77\% prompt-based (10-point gap validates Theorem~\ref{thm:gap}) \citep{wang2026agentspec}.
...
\paragraph{Software defect taxonomies.} IEEE~1044 \citep{ieee1044} provides multi-attribute anomaly classification assuming the failure-defect-fault chain. ODC \citep{chillarege1992} offers eight orthogonal defect types with development-phase correlations. Beizer \citep{beizer1990} provides a ten-category bug taxonomy. CWE covers security. None captures accretion defects (Section~\ref{sec:degradation}).

\paragraph{Software reliability.} Musa's software reliability models \citep{musa1987} connect operational profiles to defect discovery. Fenton and Neil \citep{fenton1999} introduced Bayesian networks for defect prediction. Capers Jones \citep{capersjones2012} provided the foundational DRE data (25,000 projects). Littlewood and Strigini \citep{littlewood1993, littlewood2000} proved that diverse software versions do not fail independently, a result we adapt to LLM judge-producer pairs.
...
\paragraph{Defense in depth.} The Swiss Cheese Model \citep{reason1990} frames accident causation through aligned holes in multiple barriers. Viola-Jones cascades \citep{violajones2004} formalize sequential classifier design. Boehm's cost escalation curve \citep{boehm1981} and Capers Jones' DRE composition \citep{capersjones2012} provide the economic foundation, though the cost curve may be flatter in CI/CD environments.

\paragraph{Information theory in software.} Hassan's change entropy \citep{hassan2009} predicts faults from change scattering. Our codebase entropy (Definition~\ref{def:entropy}) shifts from measuring change scattering to measuring the coupling structure that necessitates it.
...
\begin{enumerate}[leftmargin=2em]
\item \textbf{Structural before probabilistic.} Everything expressible as a formal property should be structurally enforced (Theorem~\ref{thm:gap}). This is not a preference; it is an optimality result.

\item \textbf{Cheap layers first.} Order by ROI (Theorem~\ref{thm:ordering}). Layer~1 at \$0.50/PR before Layer~4 at \$80/PR.

\item \textbf{Diversity over strength.} A weak independent detector outperforms a strong correlated one (Corollary~\ref{cor:diversity}). Cross-model judging (Claude judging GPT output) over same-model.
...
\item \textbf{Maintain the refactoring ratio.} The 2.5$\times$ violation of the equilibrium condition (Corollary~\ref{cor:refactoring}) is the root cause of codebase degradation. Any quality architecture must include mechanisms to sustain $r \geq 0.20$.
...
\item \textbf{Three diverse layers for 95\% DRE.} The copula model confirms Capers Jones: no single technique suffices, and correlated pairs underperform (Proposition~\ref{prop:jones}).
\end{enumerate}
...
\emph{``Slop is acceptable.''} Our framework agrees, partially. The optimal quality level (Theorem~\ref{thm:optquality}) is explicitly $q^* < 1$ for low-stakes code. The contribution is making this precise: for prototypes, Layer~1 only; for production, all four layers.
...
\emph{``LLM-as-Judge is unreliable.''} Partially validated. Inter-rater omega of 0.462--0.803 is concerning \citep{llmjudgetrust2024}. Same-model judging amplifies correlated errors (Theorem~\ref{thm:fkg}). But cross-model judging with structural scaffolding (Layer~3 constrained by Layers~1--2) remains cost-effective for semi-decidable types.
...
\begin{itemize}[leftmargin=2em]
\item The detection probability estimates (Table~\ref{tab:detection}) have heterogeneous confidence. No study directly measures all 18 types across all 4 layers.
...
\item Codebase entropy (Definition~\ref{def:entropy}) is an idealized metric. Computing $|\mu(f_i)|$ exactly requires solving dependency analysis, which is itself subject to Rice's theorem for semantic dependencies.
...
% BIBLIOGRAPHY
% ============================================================
\bibliography{quality-architecture}
